...
export type ChargeRecalcSerialized = {
...
const FIELD_KEYS = [
  ...
  "start_battery_level",
  "end_battery_level",
] as const satisfies readonly (keyof ChargeRecalcSerialized)[];

export function ChargeRecalcPanel({
  processId,
  computeAction,
  applyAction,
  readOnly,
  ...
  computeAction: (id: number) => Promise<{
    ok: boolean;
    error?: string;
    before?: ChargeRecalcSerialized;
    after?: ChargeRecalcSerialized;
  }>;
  applyAction: (
    id: number,
    after: ChargeRecalcSerialized,
  ...
  const t = useTranslations("charges.recalc");
  const tCommon = useTranslations("common");
  const format = useFormatter();
  const [isComputing, startCompute] = useTransition();
  const [isApplying, startApply] = useTransition();
  const [result, setResult] = useState<
    | { before: ChargeRecalcSerialized; after: ChargeRecalcSerialized }
    | null
  >(null);

  function handleCompute() {
    startCompute(async () => {
      const r = await computeAction(processId);
  ...
  async function handleApply() {
    if (!result) return;
    startApply(async () => {
      const r = await applyAction(processId, result.after);
      if (!r.ok) {
        toast.error(r.error ?? tCommon("errorOccurred"));
        return;
      }
  ...
  function fmt(key: keyof ChargeRecalcSerialized, value: string | number | null) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button type="button" onClick={handleCompute} disabled={isComputing || readOnly}>
              ...
              {FIELD_KEYS.map((key) => {
                ...
                return (
                  <TableRow key={key}>
                    ...
                    <TableCell
                      className={same ? "text-muted-foreground" : "font-semibold text-foreground"}
                    >
